...
(function() {
  var Context, ContextStack, JavaScriptCode, JavaScriptComment, Macro, Module, OppoCompileError, Splat, Symbol, clone, compile, compile_list, compile_symbol, define, define_builtin_macro, define_macro, first_item_matches, get_module, get_symbol_text, is_keyword, is_quoted, is_symbol, lambda, module_splitter, symbol, text_to_js_identifier, to_type, _ref,
  ...
  compile_list = function(ls, to_compile) {
    var c_ls, call_macro, callable, callable_is_keyword, callable_is_quoted, callable_is_symbol, result, x;
    if (to_compile == null) {
      to_compile = true;
    }
    if (ls.quoted) {
      c_ls = (function() {
        ...
        for (_i = 0, _len = ls.length; _i < _len; _i++) {
          x = ls[_i];
          _results.push((compile([(symbol('quote'))(x)]))[0]);
        }
        return _results;
      })();
      return "[" + (c_ls.join(', ')) + "]";
      ...
      if ((to_type(callable)) === "array" && !(callable_is_keyword || callable_is_quoted)) {
        callable = compile_list(callable, false);
      }
      ...
      result = call_macro.transform.apply(call_macro, ls);
      if (to_compile) {
  ...
  lambda = function() {
    var arg, args, body, c_args, c_body, context, found_splat, is_splat, normal_args, splat_arg, splat_arg_val, splat_args, var_stmt, _i, _len;
