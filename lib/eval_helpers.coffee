...
eval.def = (scope, x, RT) ->
  [__, ident, exp] = x
  oppo.types.identifier.def RT, ident, (scope.eval scope, exp)
  
eval.set = (scope, x, RT) ->
  [__, ident, exp] = x
  oppo.types.identifier["set!"] RT, ident, (scope.eval scope, exp)
...
eval.func_call = (scope, x, RT) ->
  ...
  if typeof (getValue fn) isnt 'function'
  ...
  if fn.is_macro
    getValue scope.eval scope, fn.apply scope, args
  else
    for item, i in args
